...
This file applies when Claude is working in `product-master-data-model/tests/`.
...
- dbt model contracts and schema tests (not_null, unique, relationships).
...
- dbt tests live in `src/models/schema.yml` alongside the models they test.
...
- Good: `test_gtin_check_digit_when_input_is_twelve_digits`.
